...
import stpredictions.OK3._tree

from stpredictions.OK3._classes import OK3Regressor
from stpredictions.OK3._classes import ExtraOK3Regressor

from stpredictions.OK3._tree import TREE_LEAF, TREE_UNDEFINED
from stpredictions.OK3._classes import CRITERIA
...
SPARSE_TREES = ["OK3Regressor", "ExtraOK3Regressor"]
...
def assert_tree_equal(d, s, message):
    ...
    assert_array_equal(d.y, s.y, 
                       message + ": inequal training output matrix y")
    assert_array_equal(d.K_y, s.K_y, 
                       message + ": inequal trining output Gtram matrix K_y")
...
def test_classification_toy():
    ...
    for name, Tree in OKTREES.items():
        ...
        clf = Tree(max_features=1, random_state=1, kernel="gini_clf")
        clf.fit(X, y)
        assert_array_equal(clf.predict(T), true_result,
                            "Failed with {0}".format(name))
...
def test_regression_toy():
    ...
    for name, Tree in OKTREES.items():
        ...
        clf = Tree(max_features=1, random_state=1, kernel="mse_reg")
        clf.fit(X, y)
        assert_almost_equal(reg.predict(T), true_result,
                            err_msg="Failed with {0}".format(name))
...
def test_xor():
    ...
    for name, Tree in OKTREES.items():
        ...
        clf = Tree(random_state=0, max_features=1, kernel="gini_clf")
        clf.fit(X, y)
        assert clf.score(X, y) == 1.0, "Failed with {0}".format(name)
...
def test_iris():
    ...
    for (name, Tree), criterion in product(OKTREES.items(), CRITERIONS):
        ...
        clf = Tree(criterion=criterion, max_features=2, random_state=0, kernel="gini_clf")
        clf.fit(iris.data, iris.target)
        score = accuracy_score(clf.predict(iris.data), iris.target)
        assert score > 0.5, (
            "Failed with {0}, criterion = {1} and score = {2}"
...
# La construction d'une matrice de Gram de taille supérieure à 1000 est bien trop long
...
def test_arrayrepr():
    ...
    for name, Tree in OKTREES.items():
        reg = Tree(max_depth=None, random_state=0, kernel="mse_reg")
        reg.fit(X, y)
...
def test_pure_set():
    ...
    for name, TreeClassifier in OKTREES.items():
        clf = TreeClassifier(random_state=0, kernel="gini_clf")
        clf.fit(X, y)
        assert_array_equal(clf.predict(X), y,
                            err_msg="Failed with {0}".format(name))

    for name, TreeRegressor in OKTREES.items():
        reg = TreeRegressor(random_state=0, kernel="mse_reg")
        reg.fit(X, y)
...
def test_numerical_stability():
    ...
    with np.errstate(all="raise"):
        for name, Tree in OKTREES.items():
            reg = Tree(random_state=0, kernel="mse_reg")
            reg.fit(X, y)
            reg.fit(X, -y)
            reg.fit(-X, y)
            reg.fit(-X, -y)
...
def test_importances():
    ...
    clf2 = OK3Regressor(random_state=0,
                        max_leaf_nodes=len(iris.data), kernel="gini_clf")
...
def test_importances_gini_equal_mse():
    ...
    reg = OK3Regressor(max_depth=5,
                                random_state=0, kernel="mse_reg").fit(X, y)
...
def test_max_features():
    ...
    for name, TreeClassifier in OKTREES.items():
        clf = TreeClassifier(max_features="auto", kernel="gini_clf")
        clf.fit(iris.data, iris.target)
        assert clf.max_features_ == 2

    for name, TreeEstimator in OKTREES.items():
        for kernel in ["gini_clf", "mse_reg"]:
            est = TreeEstimator(max_features="sqrt", kernel=kernel)
            est.fit(iris.data, iris.target)
            assert (est.max_features_ ==
                    int(np.sqrt(iris.data.shape[1])))
    
            est = TreeEstimator(max_features="log2", kernel=kernel)
            est.fit(iris.data, iris.target)
            assert (est.max_features_ ==
                    int(np.log2(iris.data.shape[1])))
    
            est = TreeEstimator(max_features=1, kernel=kernel)
            est.fit(iris.data, iris.target)
            assert est.max_features_ == 1
    
            est = TreeEstimator(max_features=3, kernel=kernel)
            est.fit(iris.data, iris.target)
            assert est.max_features_ == 3
    
            est = TreeEstimator(max_features=0.01, kernel=kernel)
            est.fit(iris.data, iris.target)
            assert est.max_features_ == 1
    
            est = TreeEstimator(max_features=0.5, kernel=kernel)
            est.fit(iris.data, iris.target)
            assert (est.max_features_ ==
                    int(0.5 * iris.data.shape[1]))
    
            est = TreeEstimator(max_features=1.0, kernel=kernel)
            est.fit(iris.data, iris.target)
            assert est.max_features_ == iris.data.shape[1]
    
            est = TreeEstimator(max_features=None, kernel=kernel)
            est.fit(iris.data, iris.target)
            assert est.max_features_ == iris.data.shape[1]
    
            # use values of max_features that are invalid
            est = TreeEstimator(max_features=10, kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
            ...
            est = TreeEstimator(max_features=0.0, kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
    
            est = TreeEstimator(max_features=1.5, kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
    
            est = TreeEstimator(max_features="foobar", kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
...
def test_error():
    ...
    for name, TreeEstimator in OKTREES.items():
        for kernel in ["gini_clf", "mse_reg"]:
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_leaf=-1, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_leaf=.6, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_leaf=0., kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_leaf=3., kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_weight_fraction_leaf=-1, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_weight_fraction_leaf=0.51, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_split=-1, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_split=0.0, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_split=1.1, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_samples_split=2.5, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(max_depth=-1, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(max_features=42, kernel=kernel).fit(X, y)
            # min_impurity_split warning
            with ignore_warnings(category=FutureWarning):
                with pytest.raises(ValueError):
                    TreeEstimator(min_impurity_split=-1.0, kernel=kernel).fit(X, y)
            with pytest.raises(ValueError):
                TreeEstimator(min_impurity_decrease=-1.0, kernel=kernel).fit(X, y)
    
            # Wrong dimensions
            est = TreeEstimator(kernel=kernel)
            y2 = y[:-1]
            with pytest.raises(ValueError):
                est.fit(X, y2)
            ...
            clf = TreeEstimator(kernel=kernel)
            clf.fit(X, y)
            with pytest.raises(ValueError):
                clf.predict(Xt)
            with pytest.raises(ValueError):
...
def test_min_samples_split():
    ...
    for max_leaf_nodes, name in product((None, 1000), OKTREES.keys()):
        for kernel in ["gini_clf", "mse_reg"]:
        ...
        for max_leaf_nodes, frac in product((None, 1000), np.linspace(0, 0.5, 6)):
            ...
            assert (
                ...
                total_weight * est.min_weight_fraction_leaf), (
                    "Failed with {0} min_weight_fraction_leaf={1}, kernel={2}".format(
                        name, est.min_weight_fraction_leaf, kernel))
...
def test_min_impurity_decrease():
    ...
    for max_leaf_nodes, name in product((None, 1000), OKTREES.keys()):
        ...
        for kernel in ["gini_clf", "mse_reg"]:
            ...
            est1 = TreeEstimator(max_leaf_nodes=max_leaf_nodes, random_state=0, kernel=kernel)
            # Check with explicit value of 0.05
            est2 = TreeEstimator(max_leaf_nodes=max_leaf_nodes,
                                  min_impurity_decrease=0.05, random_state=0, kernel=kernel)
            # Check with a much lower value of 0.0001
            est3 = TreeEstimator(max_leaf_nodes=max_leaf_nodes,
                                  min_impurity_decrease=0.0001, random_state=0, kernel=kernel)
            # Check with a much lower value of 0.1
            est4 = TreeEstimator(max_leaf_nodes=max_leaf_nodes,
                                  min_impurity_decrease=0.1, random_state=0, kernel=kernel)
    ...
    for name, TreeEstimator in OKTREES.items():
        for kernel in ["gini_clf", "mse_reg"]:
            ...
            assert_tree_equal(est.tree_, est2.tree_, "The trees are different after pickling.")
            score2 = est2.score(X, y)
            assert score == score2, (
                "Failed to generate same score  after pickling "
                "with {0} and kernel = {1}".format(name, kernel))
    
            for attribute in fitted_attribute:
                assert (getattr(est2.tree_, attribute) ==
                        fitted_attribute[attribute]), (
                            "Failed to generate same attribute {0} after "
                            "pickling with {1} and kernel = {2}".format(attribute, name, kernel))
...
def test_sample_weight():
    ...
    clf = OK3Regressor(max_depth=1, random_state=0, kernel="gini_clf")
    clf.fit(X, y, sample_weight=sample_weight)
    ...
    clf = OK3Regressor(random_state=1, kernel="gini_clf")
...
# # DEMANDE UN ATTRIBUT 'class_weight'  A L'ARBRE : CE N'EST PAS PRÉVU PAR OK3
# # MAIS PEUT ETRE INCORPORÉ DANS LE CALCUL DE LA MATRICE DE GRAM EN ATTRIBUANT 
# # DES POIDS DIFFERENTS SELON LES VALEURS DE Y
# def check_class_weights(name):
#     """Check class_weights resemble sample_weights behavior."""
#     TreeClassifier = OKTREES[name]

#     # Iris is balanced, so no effect expected for using 'balanced' weights
#     clf1 = TreeClassifier(random_state=0, kernel="gini_clf")
#     clf1.fit(iris.data, iris.target)
#     clf2 = TreeClassifier(class_weight='balanced', random_state=0, kernel="gini_clf")
#     clf2.fit(iris.data, iris.target)
...
#                                         {0: 2., 1: 1., 2: 2.},
#                                         {0: 1., 1: 2., 2: 2.}],
#                           random_state=0, kernel="gini_clf")
#     clf3.fit(iris.data, iris_multi)
#     assert_almost_equal(clf2.feature_importances_, clf3.feature_importances_)
#     # Check against multi-output "auto" which should also have no effect
#     clf4 = TreeClassifier(class_weight='balanced', random_state=0, kernel="gini_clf")
...
#     clf2 = TreeClassifier(class_weight=class_weight, random_state=0, kernel="gini_clf")
#     clf2.fit(iris.data, iris.target)
#     assert_almost_equal(clf1.feature_importances_, clf2.feature_importances_)
...
#     clf = TreeClassifier(class_weight='the larch', random_state=0, kernel="gini_clf")
#     with pytest.raises(ValueError):
#         clf.fit(X, y)
#     with pytest.raises(ValueError):
#         clf.fit(X, _y)
...
#     clf = TreeClassifier(class_weight=1, random_state=0, kernel="gini_clf")
#     with pytest.raises(ValueError):
#         clf.fit(X, _y)

#     # Incorrect length list for multi-output
#     clf = TreeClassifier(class_weight=[{-1: 0.5, 1: 1.}], random_state=0, kernel="gini_clf")
#     with pytest.raises(ValueError):
#         clf.fit(X, _y)
...
def test_max_leaf_nodes():
    ...
    for name, TreeEstimator in OKTREES.items():
        for kernel in ["gini_clf", "mse_reg"]:
            est = TreeEstimator(max_depth=None, max_leaf_nodes=k + 1, kernel=kernel).fit(X, y)
            assert est.get_n_leaves() == k + 1
    
            # max_leaf_nodes in (0, 1) should raise ValueError
            est = TreeEstimator(max_depth=None, max_leaf_nodes=0, kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
            est = TreeEstimator(max_depth=None, max_leaf_nodes=1, kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
            est = TreeEstimator(max_depth=None, max_leaf_nodes=0.1, kernel=kernel)
            with pytest.raises(ValueError):
                est.fit(X, y)
...
def test_max_leaf_nodes_max_depth():
    ...
    for name, TreeEstimator in OKTREES.items():
        for kernel in ["gini_clf", "mse_reg"]:
            est = TreeEstimator(max_depth=1, max_leaf_nodes=k, kernel=kernel).fit(X, y)
...
def test_arrays_persist():
    ...
    for attr in [#'y', 'leaf_to_train_exs', 'candidates',
        ...
        value = getattr(OK3Regressor(kernel="gini_clf").fit([[0], [1]],
                                                      [0, 1]).tree_, attr)
    ...
    for name, TreeEstimator in OKTREES.items():
        for kernel in ["gini_clf", "mse_reg"]:
            ...
            if "Extra" not in name:
                est = TreeEstimator(random_state=0, max_features=1, kernel=kernel)
                est.fit(X, y)
...
def test_with_only_one_non_constant_features():
    ...
    # for name, TreeEstimator in OKTREES.items():
    #     est = TreeEstimator(random_state=0, max_features=1, kernel="gini_clf")
    #     est.fit(X, y)
    #     assert est.tree_.max_depth == 1
    #     assert_array_equal(est.predict_proba(X), np.full((4, 2), 0.5))

    for name, TreeEstimator in OKTREES.items():
        est = TreeEstimator(random_state=0, max_features=1, kernel="mse_reg")
        est.fit(X, y)
        assert est.tree_.max_depth == 1
...
def test_huge_allocations():
    ...
    huge = 2 ** (n_bits + 1)
    clf = OK3Regressor(splitter='best', max_leaf_nodes=huge, kernel="gini_clf")
...
def check_sparse_input(tree, dataset, max_depth=None, only_reg=False):
    ...
    for sparse_format in (csr_matrix, csc_matrix, coo_matrix):
        ...
        for kernel in ["gini_clf", "mse_reg"]:
            if ((not only_reg) or ("reg" in kernel)):
                # Check the default (depth first search)
                d = TreeEstimator(random_state=0, max_depth=max_depth, kernel=kernel).fit(X, y)
                s = TreeEstimator(random_state=0, max_depth=max_depth, kernel=kernel).fit(X_sparse, y)
...
def check_sparse_parameters(tree, dataset):
    ...
    for kernel in ["gini_clf", "mse_reg"]:
        # Check max_features
        d = TreeEstimator(random_state=0, max_features=1, max_depth=2, kernel=kernel).fit(X, y)
        s = TreeEstimator(random_state=0, max_features=1, max_depth=2, kernel=kernel).fit(X_sparse, y)
        assert_tree_equal(d.tree_, s.tree_,
                          "{0} with dense and sparse format gave different "
                          "trees".format(tree))
        ...
        d = TreeEstimator(random_state=0, max_features=1,
                          min_samples_split=10, kernel=kernel).fit(X, y)
        s = TreeEstimator(random_state=0, max_features=1,
                          min_samples_split=10, kernel=kernel).fit(X_sparse, y)
        assert_tree_equal(d.tree_, s.tree_,
                          "{0} with dense and sparse format gave different "
                          "trees".format(tree))
        assert_array_almost_equal(s.predict(X), d.predict(X))
        ...
        d = TreeEstimator(random_state=0,
                          min_samples_leaf=X_sparse.shape[0] // 2, kernel=kernel).fit(X, y)
        s = TreeEstimator(random_state=0,
                          min_samples_leaf=X_sparse.shape[0] // 2, kernel=kernel).fit(X_sparse, y)
        assert_tree_equal(d.tree_, s.tree_,
                          "{0} with dense and sparse format gave different "
                          "trees".format(tree))
        assert_array_almost_equal(s.predict(X), d.predict(X))
        ...
        d = TreeEstimator(random_state=0, max_leaf_nodes=3, kernel=kernel).fit(X, y)
        s = TreeEstimator(random_state=0, max_leaf_nodes=3, kernel=kernel).fit(X_sparse, y)
        assert_tree_equal(d.tree_, s.tree_,
                          "{0} with dense and sparse format gave different "
                          "trees".format(tree))
        assert_array_almost_equal(s.predict(X), d.predict(X))
    ...
    for criterion in CRITERIONS:
        for kernel in ["gini_clf", "mse_reg"]:
            d = TreeEstimator(random_state=0, max_depth=3,
                              criterion=criterion, kernel=kernel).fit(X, y)
            s = TreeEstimator(random_state=0, max_depth=3,
                              criterion=criterion, kernel=kernel).fit(X_sparse, y)
...
def check_raise_error_on_1d_input(name):
    ...
    for kernel in ["gini_clf", "mse_reg"]:
        with pytest.raises(ValueError):
            TreeEstimator(random_state=0, kernel=kernel).fit(X, y)
...
def _check_min_weight_leaf_split_level(TreeEstimator, X, y, sample_weight):
    for kernel in ["gini_clf", "mse_reg"]:
        ...
        est = TreeEstimator(random_state=0, min_weight_fraction_leaf=0.4, kernel=kernel)
        est.fit(X, y, sample_weight=sample_weight)
        assert est.tree_.max_depth == 0
...
def check_decision_path(name):
    ...
    for kernel in ["gini_clf", "mse_reg"]:
        TreeEstimator = OKTREES[name]
        est = TreeEstimator(random_state=0, max_depth=2, kernel=kernel)
        est.fit(X, y)
...
def test_criterion_copy():
    ...
    for copy_func in [copy.copy, copy.deepcopy, _pickle_copy]:
        ...
        for _, typename in CRITERIA.items():
            criteria = typename(n_samples)
            result = copy_func(criteria).__reduce__()
            typename_, (n_samples_,), _ = result
...
def test_empty_leaf_infinite_threshold():
    ...
    for X in [X_full, X_sparse]:
        tree = OK3Regressor(random_state=0, kernel="mse_reg").fit(X, y)
...
@pytest.mark.parametrize(
    "tree_cls", OKTREES.values())
def test_prune_tree_classifier_are_subtrees(criterion, dataset, tree_cls):
    dataset = DATASETS[dataset]
    X, y = dataset["X"], dataset["y"]
    est = tree_cls(max_leaf_nodes=20, random_state=0, kernel="gini_clf")
...
def test_prune_tree_regression_are_subtrees(criterion, dataset, tree_cls):
    ...
    est = tree_cls(max_leaf_nodes=20, random_state=0, kernel="mse_reg")
    info = est.cost_complexity_pruning_path(X, y)
...
def test_prune_single_node_tree():
    ...
    clf2 = OK3Regressor(random_state=0, ccp_alpha=10, kernel="gini_clf")
    clf2.fit([[0], [1]], [0, 0])
...
def assert_pruning_creates_subtree(estimator_cls, X, y, pruning_path, kernel):
    ...
    for ccp_alpha in pruning_path:
        est = estimator_cls(
            max_leaf_nodes=20, ccp_alpha=ccp_alpha, random_state=0, kernel=kernel).fit(X, y)
...
def assert_is_subtree(tree, subtree):
    ...
    assert_array_equal(tree.y, subtree.y)
    assert_array_equal(tree.K_y, subtree.K_y)
...
def test_X_idx_sorted_deprecated(TreeEstimator):
    ...
    for kernel in ["gini_clf", "mse_reg"]:
        tree = TreeEstimator(kernel=kernel)
